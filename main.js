...
$(document).ready(function () {
  ...
  gallerySlider.on('translated.owl.carousel', function(event) {
    ...
    var currentItemIndex = event.item.index; // Index of the current item

    // Check if the current item is the last one
    if (currentItemIndex === items - 1) {
      // If it's the last item, reset the carousel to the first item
      gallerySlider.trigger('to.owl.carousel', 0);
    }
  });
});
